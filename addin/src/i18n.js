...
export const STRINGS = {
  en: {
    ...
    hostLine: 'Host: {app} {version} ({platform}) · insert: {insert} · baseline shift: {shift} · fields: {fields}',
    hostYes: 'yes', hostNo: 'no', insertSvg: 'vector SVG', insertPng: 'raster PNG', insertNone: 'unavailable',
    noBaselineShift: 'Inline equations will sit slightly high on this Word build ({version}): the baseline-shift API (WordApiDesktop 1.3) needs Word for Microsoft 365 version 2507 or later on Windows, 16.99 on Mac; it is not available in LTSC or on the web.',
    baselineShiftFailed: 'Equation inserted, but lowering it to the text baseline failed: {error}',
  ...
  de: {
    ...
    hostLine: 'Host: {app} {version} ({platform}) · Einfügen: {insert} · Grundlinienversatz: {shift} · Felder: {fields}',
    hostYes: 'ja', hostNo: 'nein', insertSvg: 'Vektor-SVG', insertPng: 'Raster-PNG', insertNone: 'nicht verfügbar',
    noBaselineShift: 'Inline-Gleichungen sitzen in dieser Word-Version ({version}) etwas zu hoch: die Grundlinien-API (WordApiDesktop 1.3) braucht Word für Microsoft 365 ab Version 2507 (Windows) bzw. 16.99 (Mac); in LTSC und im Web gibt es sie nicht.',
    baselineShiftFailed: 'Gleichung eingefügt, aber das Absenken auf die Grundlinie ist fehlgeschlagen: {error}',
